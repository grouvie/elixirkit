//! Internal runtime and transport support for the public `PubSub` API.
//!
//! This module keeps today's TCP framed transport and startup behavior intact,
//! but gives the crate a single private place to evolve bridge-core internals
//! without changing the public `PubSub` surface yet.
...
/// Internal runtime/core owner behind the public `PubSub` compatibility layer.
#[expect(
    clippy::redundant_pub_crate,
    reason = "Runtime stays crate-visible for the internal seam while the module itself remains private"
)]
#[derive(Clone)]
...
impl Runtime {
    pub(crate) fn listen(url: &str) -> Result<Self, io::Error> {
    ...
    pub(crate) fn connect(url: &str) -> Result<Self, io::Error> {
        let port = parse_url(url)?;
    ...
    pub(crate) fn subscribe<F>(&self, topic: &str, callback: F)
    where
    ...
    pub(crate) fn broadcast(&self, topic: &str, message: &[u8]) -> io::Result<()> {
